...
open class BadgeButton: UIButton {
    ...
    fileprivate func setupBadgeViewWithString(badgeText: String?) {
        ...
        var xVal: CGFloat = 0, yVal: CGFloat = 0
        ...
        calculateXYForBadge(xInput: &xVal, yInput: &yVal, badgeSize: CGSize(width: width, height: height))

        //add badgeEdgeInset
        xVal += horizontal
        yVal += vertical

        badgeLabel.frame = CGRect(x: xVal, y: yVal, width: width, height: height)
     ...
     **/
    fileprivate func calculateXYForBadge(xInput : inout CGFloat, yInput : inout CGFloat, badgeSize: CGSize) {

        switch self.badgeAnchor {

        case .topLeft(let topOffset, let leftOffset):
            xInput = -badgeSize.width/2 + leftOffset
            yInput = -badgeSize.height/2 + topOffset

        case .topRight(let topOffset, let rightOffset):
            xInput = self.bounds.size.width - badgeSize.width/2 + rightOffset
            yInput = -badgeSize.height/2 + topOffset

        case .bottomLeft(let bottomOffset, let leftOffset):
            xInput = -badgeSize.width/2 + leftOffset
            yInput = self.bounds.size.height - badgeSize.height/2 + bottomOffset
        ...
        case .center:
            xInput = self.bounds.size.width/2 - badgeSize.width/2
            yInput = self.bounds.size.height/2 - badgeSize.height/2
